...
def data_base_mnist():
    ...
    # Load Dataset
    mnist_dataset = torchvision.datasets.MNIST(root='datasets/',train=True,transform=norm_transform,download=True)
    test_dataset = torchvision.datasets.MNIST(root='datasets/',train=False,transform=norm_transform)

    # Prepare the training and validation splits
    dataset_mask = list(range(num_training))
    train_dataset = torch.utils.data.Subset(mnist_dataset, dataset_mask)
    dataset_mask = list(range(num_training, num_training + num_validation))
    val_dataset = torch.utils.data.Subset(mnist_dataset, dataset_mask)
